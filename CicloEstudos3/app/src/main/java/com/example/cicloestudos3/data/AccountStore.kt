...
/**
 * Local-only account store (no backend). Credentials are kept in SharedPreferences
 * with the password stored as a SHA-256 hash rather than plain text.
 */
class AccountStore(context: Context) {

    private val prefs = context.applicationContext
        .getSharedPreferences("estudos_accounts", Context.MODE_PRIVATE)
